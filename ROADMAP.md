...
**Current state:** One repository, two workstreams. **`master`** is `pertype` — released and
published (v0.1.0 on GitHub Releases, PyPI, crates.io; dual-licensed AGPL-3.0 + commercial),
`TODO.md` 68 done / 3 low-priority open, productization plan COMPLETE, data-type sweep backlog
exhausted. **`flight-core`** is `libpfc` — a freestanding C99 flight core with 8 green CI jobs,
CBMC proofs, a traceability matrix and an independent Python ground decoder, which has **never been
merged or released**. The branches have now diverged (master +3, flight-core +42).
...
**Last surveyed:** 2026-08-15 · **Decisions recorded:** 2026-08-15 review
...
- **Does `ROADMAP.md` become a tracked file after the merge?** It sits at the repo root alongside
  gitignored `HANDOFF.md`/`RESUME.md`. Post-merge there is one branch, so the only question is
  tracked-or-not. I'd suggest tracking it.
- **No horizon or constraint was specified** (the optional switches block was left intact). I have
  assumed no deadline and solo working. A stated horizon would mainly change whether M3 is done in
  one push or spread out.
